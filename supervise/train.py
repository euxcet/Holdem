...
class DeepStackDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.batch_size, num_workers=64, shuffle=True)

    def val_dataloader(self):
        return DataLoader(self.valid_dataset, batch_size=self.batch_size, num_workers=64, shuffle=False)
...
if __name__ == '__main__':
    ...
    trainer = Trainer(
        ...
        logger=wandb_logger,
        callbacks=[
            ModelCheckpoint(every_n_epochs=10, save_top_k=3),
        ]
