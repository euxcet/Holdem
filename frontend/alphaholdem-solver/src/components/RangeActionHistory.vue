...
const {
    ...
    current_step,
    highlight_action,
} = storeToRefs(rangeStore)

function actionMouseOver(obsIndex: number, actionIndex: number) {
    if (obsIndex == rangeStore.current_step) {
        rangeStore.highlight_action = actionIndex
    }
}

function actionMouseLeave(obsIndex: number, actionIndex: number) {
    if (actionIndex == rangeStore.highlight_action) {
        rangeStore.highlight_action = -1
    }
}

async function perform_action(obsIndex: number, actionIndex: number) {
    await rangeStore.perform_action(obsIndex, actionIndex)
}
